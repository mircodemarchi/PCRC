 ...
 * @date 11/09/2020
 * @brief Source file of CRC64 bytewise implementation.
 */
...
uint64_t crc64_bytewise(const uint8_t *buffer, size_t length, const uint64_t *lu)
{
    uint64_t crc = 0;
    for (size_t i = 0; i < length; i++)
    {
        // XOR-in next input byte.
        uint8_t data = (uint8_t)((crc ^ (((uint64_t) buffer[i]) << 56)) >> 56);
        // Get current CRC value from lookup table.
        crc = (uint64_t)(crc << 8) ^ lu[data];
    }
    return crc;
}
...
uint64_t *generate_crc64_lu(uint64_t *lu)
{
    ...
    {
        lu[i] = crc64_bitwise((uint8_t *) &i, 1);
    }
    return lu;
}
